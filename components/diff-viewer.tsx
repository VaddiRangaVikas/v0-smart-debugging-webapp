...
export function DiffViewer({ originalCode, correctedCode, diffView, hasResult, errors = [] }: DiffViewerProps) {
  ...
  // Simple computations - no hooks here
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(correctedCode || originalCode);
    setCopied(true);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([correctedCode || originalCode], { type: 'text/plain' });
  ...
  // Errors detected but no code changes
  if (noChangesNeeded && hasErrors) {
    return (
      <Card className="flex h-full flex-col border-red-500/30 bg-card/50 backdrop-blur-sm">
        <CardHeader className="pb-2">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Badge variant="outline" className="gap-1.5 border-red-500/30 bg-red-500/10 text-red-400">
              ...
              <span>Errors on lines: {errors.filter(e => e.line).map(e => e.line).join(', ')}</span>
            </Badge>
          </div>
        </CardHeader>
          ...
          <Tabs defaultValue="errors" className="flex h-full flex-col">
            <div className="border-b border-border/30 px-4">
              <TabsList className="h-9 bg-transparent p-0">
                <TabsTrigger value="errors" className="gap-1.5 rounded-none border-b-2 border-b-transparent px-4 text-xs data-[state=active]:border-b-red-500 data-[state=active]:bg-transparent data-[state=active]:text-red-400">
                  <AlertTriangle className="h-3.5 w-3.5" />
                  Error View
                </TabsTrigger>
                <TabsTrigger value="code" className="gap-1.5 rounded-none border-b-2 border-b-transparent px-4 text-xs data-[state=active]:border-b-cyan-400 data-[state=active]:bg-transparent data-[state=active]:text-cyan-400">
              ...
              <ScrollArea className="h-full">
                <div className="font-mono text-xs">
                  {codeWithErrorHighlighting.map((line, i) => (
                    <div key={i} className={cn('flex items-start gap-2 px-4 py-0.5', line.type === 'error' ? 'bg-red-500/20 border-l-2 border-l-red-500' : 'border-l-2 border-l-transparent')}>
                      <span className="flex w-5 shrink-0 items-center justify-center">
                        {line.type === 'error' && <AlertTriangle className="h-3 w-3 text-red-400" />}
                      </span>
                      <span className="w-8 shrink-0 text-right text-muted-foreground/40 select-none">{line.lineNumber}</span>
                      <span className={cn('flex-1 whitespace-pre-wrap break-all', line.type === 'error' && 'text-red-400')}>{line.content || ' '}</span>
                    </div>
                  ))}
                </div>
                  ...
                  {errors.map((error, i) => (
                    <div key={i} className="rounded-lg bg-red-500/10 border border-red-500/30 p-3">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge variant="outline" className="text-[10px] border-red-500/50 text-red-400">Line {error.line || '?'}</Badge>
                        <Badge variant="outline" className="text-[10px] border-orange-500/50 text-orange-400">{error.type}</Badge>
                      </div>
            ...
            <TabsContent value="code" className="m-0 flex-1 overflow-hidden">
              <ScrollArea className="h-full">
                <div className="font-mono text-xs">
                  {originalCode.split('\n').map((line, i) => (
                    <div key={i} className="flex items-start gap-1 px-2 py-1 min-h-[24px]">
                      <span className={cn("w-10 shrink-0 text-right select-none pr-2", errorLineNumbers.has(i + 1) ? "text-red-400 font-bold" : "text-muted-foreground/40")}>{i + 1}</span>
      ...
      <Card className="h-full border-green-500/30 bg-card/50 backdrop-blur-sm">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary" className="bg-green-500/20 text-green-400 border border-green-500/30">No Changes Needed</Badge>
          </div>
        </CardHeader>
        <CardContent className="flex h-[250px] flex-col items-center justify-center">
          <div className="relative mb-4 flex h-16 w-16 items-center justify-center">
  ...
  // Main diff view with changes
  return (
    <Card className="flex h-full flex-col border-cyan-500/30 bg-card/50 backdrop-blur-sm">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2 text-sm">
            <div className="flex h-6 w-6 items-center justify-center rounded-md bg-cyan-500/20">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <Badge variant="outline" className="gap-1.5 border-red-500/50 bg-red-500/20 text-red-400">
            <Minus className="h-3 w-3" />
            <span>{removedLines} removed</span>
          </Badge>
          <Badge variant="outline" className="gap-1.5 border-cyan-500/50 bg-cyan-500/20 text-cyan-400">
        ...
        <Tabs value={view} onValueChange={(v) => setView(v as 'diff' | 'clean')} className="flex h-full flex-col">
          <div className="border-b border-border/30 px-4">
            <TabsList className="h-10 bg-transparent p-0 gap-4">
              <TabsTrigger value="diff" className="gap-2 rounded-none border-b-2 border-b-transparent px-3 py-2 text-xs font-medium data-[state=active]:border-b-cyan-400 data-[state=active]:bg-transparent data-[state=active]:text-cyan-400">
                <Eye className="h-4 w-4" />
                Diff View
              </TabsTrigger>
              <TabsTrigger value="clean" className="gap-2 rounded-none border-b-2 border-b-transparent px-3 py-2 text-xs font-medium data-[state=active]:border-b-cyan-400 data-[state=active]:bg-transparent data-[state=active]:text-cyan-400">
          ...
          <TabsContent value="diff" className="m-0 flex-1 overflow-hidden">
            <ScrollArea className="h-full">
              <div className="font-mono text-xs">
                {diffView.map((line, i) => (
                  <div key={i} className={cn('flex items-start gap-1 px-2 py-1 min-h-[24px]', getDiffLineStyle(line.type))}>
                    <span className="flex w-6 shrink-0 items-center justify-center">{getDiffIcon(line.type)}</span>
                    <span className={cn("w-10 shrink-0 text-right select-none pr-2", line.type === 'removed' && 'text-red-400/70', line.type === 'added' && 'text-cyan-400/70', line.type === 'unchanged' && 'text-muted-foreground/40')}>{line.lineNumber}</span>
                    <span className={cn('flex-1 whitespace-pre-wrap break-all', line.type === 'removed' && 'text-red-400 line-through', line.type === 'added' && 'text-cyan-400 font-medium')}>{line.content || ' '}</span>
                  </div>
                ))}
              </div>
            </ScrollArea>
          ...
          <TabsContent value="clean" className="m-0 flex-1 overflow-hidden">
            <ScrollArea className="h-full">
              <div className="font-mono text-xs">
                {correctedCode.split('\n').map((line, i) => (
                  <div key={i} className="flex items-start gap-1 px-2 py-1 min-h-[24px] hover:bg-cyan-500/5">
                    <span className="w-10 shrink-0 text-right text-cyan-400/50 select-none pr-2">{i + 1}</span>
                    <span className="flex-1 whitespace-pre-wrap break-all">{line || ' '}</span>
                  </div>
                ))}
              </div>
